...
def _seed(conn: sqlite3.Connection) -> None:
    ...
    projects = [
        ...
        ("PULSE", "Pulse Status", "Internal reliability console for support asset operations.", "active"),
    ...
    tickets = [
        ...
        (2, "Export timeline as JSON for audit bot", "Compliance automation needs a JSON feed that mirrors the timeline UI for asset review sampling.", "review", "high", 4, 2, "2026-05-31", "2026-05-19 09:20:00", "2026-05-24 11:10:00"),
    ]
    conn.executemany(
        """
        INSERT INTO tickets (
    ...
    events = [
        ...
        (8, 4, "created", "Compliance requested an audit-exportable asset timeline.", "2026-05-19 09:20:00"),
...
def create_ticket(
    *,
    ...
    with connect(db_path) as conn:
        ...
        conn.execute(
            "INSERT INTO ticket_events (ticket_id, actor_id, event_type, details, created_at) VALUES (?, ?, ?, ?, ?)",
            (ticket_id, reporter_id, "created", "Created a new ticket from the asset operations board.", _now()),
        )
        conn.commit()
        return ticket_id
